...
function generateDownloadPath(projectId, ref, job, artifactPath) {
  let url12 = endpoint`projects/${projectId}/jobs/artifacts/${ref}`;
  if (artifactPath) {
    url12 += endpoint`/raw/${artifactPath}?job=${job}`;
  } else {
    url12 += endpoint`/download?job=${job}`;
  }
  return url12;
}
var JobArtifacts = class extends requesterUtils.BaseResource {
  downloadArchive(projectId, {
    ...
    if (jobId)
      ...
      url12 = generateDownloadPath(projectId, ref, job, artifactPath);
    else
      throw new Error(
